...
export function mode_resolve_all()
{
 ...
 * @returns {Promise}
 */
funexport ction start_character(name,code_slot_or_name)
{
...
export function command_character(name,code_snippet)
{
	// Commands the character in [CODE] mode
	parent.character_code_eval(name,code_snippet);
}

export function get_active_characters()
{
...
export function change_server(region,name) // change_server("EU","I") or change_server("ASIA","PVP") or change_server("US","III")
{
	parent.window.location.href="/character/"+character.name+"/in/"+region+"/"+name+"/";
}

export function is_in_pvp()
{
	return G.maps[character.map].pvp || server.pvp;
}

export function is_npc(entity)
{
	if(entity && (entity.npc || entity.type=="npc")) return true;
}

export function is_monster(entity)
{
	if(entity && entity.type=="monster") return true;
}

export function is_character(entity)
{
	if(entity && entity.type=="character" && !entity.npc) return true;
}

export function interact(name)
{
	if(name=="monsterhunt")
	{
...
export function enter(place,name)
{
...
export function join(event)
{
	// show_json(G.events);
	return parent.join(event);
}

export function use_nearest_door()
{
...
export function activate(num) // activates an item, likely a booster, in the num-th inventory slot
{
	return parent.activate(num);
}

export function shift(num,name) // shifts an item, likely a booster, in the num-th inventory slot
{
	// shift(0,'xpbooster')
	// shift(0,'luckbooster')
	// shift(0,'goldbooster')
...
export function use_skill(name,target?,extra_arg?)
{
...
export function reduce_cooldown(name,ms)
{
...
export function bank_deposit(gold)
{
...
export function bank_withdraw(gold)
{
	if(!character.bank) return rejecting_promise({reason:"not_in_bank"});
...
export function bank_store(num,pack,pack_num)
{
...
export function bank_retrieve(pack,pack_num,num)
{
...
export function bank_swap(pack,a,b)
{
...
export function swap(a,b) // inventory move/swap
{
	parent.socket.emit("imove",{a:a,b:b});
	return parent.push_deferred("imove");
}

export function locate_item(name)
{
...
export function item_properties(item) // example: item_properties(character.items[0])
{
	if(!item || !item.name) return null;
	return calculate_item_properties(item);
}

export function item_grade(item) // example: item_grade(character.items[0])
{
	// 0 Normal
	// 1 High
	// 2 Rare
...
export function item_value(item) // example: item_value(character.items[0])
{
...
export function pause() // Pauses the Graphics
{
	parent.pause();
}
...
export function set_message(text,color?)
{
...
export function game_log(message,color,x?)
{
...
export function safe_log(message,color)
{
...
export function get_target_of(entity) // .target is a Name for Monsters and `id` for Players - this function return whatever the entity in question is targeting
{
...
export function change_target_privately(target)
{
	parent.ctarget=target;
...
export function can_move_to(x,y)
{
...
export function xmove(x,y)
{
    if(can_move_to(x,y)) return move(x,y);
    else return smart_move({x:x,y:y});
}

export function is_in_range(target,skill)
{
...
export function is_on_cooldown(skill)
{
	// If the skill cooldown is shared with another skill, then return that:
	if(G.skills[skill] && G.skills[skill].share) return is_on_cooldown(G.skills[skill].share);
	// parent.next_skill records timestamps for the last use of a skill + cooldown. If that is in the future, then return true.
	if(parent.next_skill[skill] && new Date()<parent.next_skill[skill]) return true;
	return false;
}

export function can_attack(target)
{
...
{
	if(entity.me && smart.moving) return true; // character.me is one
	if(entity.moving) return true;
	return false;
}

export function is_transporting(entity)
{
...
export function buy(name,quantity) //item names can be spotted from show_json(character.items) - they can be bought only if an NPC sells them
{
	return parent.buy(name,quantity); // returns a Promise
}

export function buy_with_gold(name,quantity)
{
	return parent.buy_with_gold(name,quantity); // returns a Promise
}

export function buy_with_shells(name,quantity)
{
	// returns {success:false,in_progress:true}
	return parent.buy_with_shells(name,quantity);
}

export function sell(num,quantity) //sell an item from character.items by it's order - 0 to N-1
{
	return parent.sell(num,quantity);
}

export function consume(num) // consumes or uses an inventory item
{
	parent.socket.emit("equip",{num:num,consume:true});
	return parent.push_deferred("equip");
}

export function equip(num,slot) // slot is optional
{
	if(num<0)
	{
...
export function trade_buy(target,trade_slot,quantity)
{
...
export function trade_sell(target,trade_slot,quantity)
{
...
export function wishlist(trade_slot,name,price,level,quantity)
{
	// where trade_slot is 1 to 16
...
export function giveaway(slot,num,q,minutes)
{
...
export function join_giveaway(name,slot,rid)
{
...
export function upgrade(item_num,scroll_num,offering_num,only_calculate) // number of the item and scroll on the show_json(character.items) array - 0 to N-1
{
	return parent.upgrade(item_num,scroll_num,offering_num,"code",only_calculate); // returns a Promise
}

export function compound(item0,item1,item2,scroll_num,offering_num,only_calculate) // for example -> compound(0,1,2,6) -> 3 items in the first 3 slots, scroll at the 6th spot
{
	return parent.compound(item0,item1,item2,scroll_num,offering_num,"code",only_calculate); // returns a Promise
}

export function craft(i0,i1,i2,i3,i4,i5,i6,i7,i8)
...
async function exchange(item_num)
{
	...
	return {success:true,reward:name}
}

export function exchange_buy(token,name)
{
	// Example: exchange_buy('funtoken','confetti')
	return parent.exchange_buy(token,name);
}

export function say(message) // please use MORE responsibly, thank you! :)
{
	return parent.say(message,safeties);
}

export function party_say(message)
{
	return parent.party_say(message,safeties);
}

export function pm(name,message)
{
...
export function equip_cx(slot,cx_name)
{
...
export function show_json(e) // renders the object as json inside the game
{
...
export function get_monster(id)
{
...
export function find_npc(npc_id)
{
...
export function get_nearest_hostile(args) // mainly as an example [08/02/17]
{
	var min_d=999999,target=null;
...
export function open_stand(num)
{
...
export function close_stand()
{
	return parent.close_merchant();
}

export function send_gold(receiver,gold)
{
	if(!receiver)
	{
...
export function send_item(receiver,num,quantity)
{
	if(!receiver)
	{
...
export function send_cx(receiver,cx)
{
...
export function send_mail(to,subject,message,item)
{
	// returns {success:false,in_progress:true}
...
export function destroy(num) // num: 0 to 41
{
	parent.p_item=num;
	return parent.poof(1);
}

export function send_party_invite(name,is_request) // name could be a player object, name, or id
{
...
export function kick_party_member(name)
{
	parent.socket.emit('party',{event:'kick',name:name});
	return parent.push_deferred("party");
}

export function accept_magiport(name)
{
...
export function unfriend(name) // instead of a name, an owner id also works, this is currently the only way to unfriend someone [20/08/18]
{
...
export function handle_command(command,args) // command's are things like "/party" that are entered through Chat - args is a string
{
...
export function send_server_cm(to,message)
{
	// to: Name or Array of Name's
	// message: JSON object
...
export function on_disappear(entity,data)
{
	// game_log("disappear: "+entity.id+" "+JSON.stringify(data));
}

export function on_party_invite(name) // called by the inviter's name
{
	// accept_party_invite(name)
}

export function on_party_request(name) // called by the inviter's name - request = someone requesting to join your existing party
{
	// accept_party_request(name)
}

export function on_magiport(name) // called by the mage's name in PVE servers, in PVP servers magiport either succeeds or fails without consent
{
	// accept_magiport(name)
}

export function on_map_click(x,y)
{
...
export function on_destroy() // called just before the CODE is destroyed
{
	clear_drawings();
	clear_buttons();
}

export function on_draw() // the game calls this function at the best place in each game draw frame, so if you are playing the game at 60fps, this function gets called 60 times per second
{
...
export function draw_line(x,y,x2,y2,size,color)export 
{
...
export function draw_circle(x,y,radius,size,color) 
{
	if(!game.graphics) return;
...
export function clear_drawings()
{
...
export function add_top_button(id,value,fn)
{
	if(!buttons[id])
	{
...
export function add_bottom_button(id,value,fn)
{
	if(!buttons[id])
	{
...
export function set_button_value(id,value)
{
	parent.$(".codebutton"+id).html(value);
}

export function set_button_color(id,color)
{
	parent.$(".codebutton"+id).css("border-color",color);
}

export function set_button_onclick(id,fn)
{
	buttons[id].fn=fn;
}

export function clear_buttons()
{
...
export function trigger_character_event(name,data)
{
	character.trigger(name,data);
}

export function trigger_event(name,data)
{
	game.trigger(name,data);
}

export function preview_item(def,args)
{
...
export function set_skillbar() // example: set_skillbar("1","2","3","4","R") or set_skillbar(["1","2","3","4","R"])
{
	var arr=["1"];
...
export function set_keymap(keymap) // example: {"1":{"name":"use_mp"},"2":{"name":"use_hp"}}
{
...
export function map_key(key,skill,code) // example: map_key("1","use_hp") or map_key("2","snippet","say('OMG')") or map_key("1","esc") or map_key("ESC","up")
{
...
export function reset_mappings()
{
	parent.keymap={};
	parent.skillbar=[];
...
export function local_cm_logic()
{
...
var local_m_num=0;
function send_locaexport l_cm(name,data)
{
...
export function is_character_local(name)
{
...
export function pset(name,value)
{
...
export function load_code(name_or_slot,onerror)
{
...
export function require_code(name_or_slot)
{
...
export function upload_code(slot_number,slot_name,code_string)
{
...
function get_active_code_slot()export 
{
	return active_code_slot;
}

export function get_edited_code_slot()
{
...
export function smart_move(destination,on_done)
{
...
export function start_pathfinding()
{
...
export function cli_smart_move_result(data)
{
	if(data.found)
	{
		smart.found=true;
...
export function continue_pathfinding()
{
	bfs();
}

export function smart_move_logic()
{
...
export function eval_s(code) // this is how snippets are eval'ed if they include "output="/"json_output=" - so if they include these, the scope of eval isn't global - doesn't matter much [13/07/18]
{
...
export function performance_trick()
{
