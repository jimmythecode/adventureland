import { is_on_cooldown, use_hp_or_mp, use_skill } from "../adventureland/runner_functions";
...
export async function resolving_promise(args: {reason:"safeties" |"cooldown" | "full",success:false,used?:false}){
...
export function optimisePotions()
{
	...
	if(is_on_cooldown("use_hp")) return resolving_promise({success:false,reason:"cooldown"}); // Potions/heals share a cooldown, so return out if we're on a cooldown
	...
	else if(max_mp - mp > 150) return use_skill('regen_mp'); // (regen doesn't use items, but heals less and has a longer cooldown)
	else if(max_mp - mp > 150) return use_skill('regen_mp'); 
	else used=false;
	if(used)
